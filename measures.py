...
def srmr(wav):
    return _srmr.srmr(wav)[0]

# def dvec(wav):
#     if Path("dvec_pca.pkl").exists():
#         with open("dvec_pca.pkl", "rb") as f:
#             pca = pickle.load(f)
#     else:
#         raise Exception("dvec_pca.pkl not found")
#     with torch.inference_mode():
#         return _dvector.embed_utterance(_wav2mel(wav, _fs)).numpy()

def dvec(dvec):
    return dvec

def f0(wav):
    _f0, t = pw.dio(wav.astype("double"), _fs)
    f0 = pw.stonemask(wav.astype("double"), _f0, t, _fs)
    return np.mean(f0)

def energy(wav):
    wav_len = len(wav) / _fs
    return np.mean([np.sqrt(np.sum(x**2)) for x in np.array_split(wav, wav_len * 50)])

def _duration_meta(meta_file):
...
def _duration_textgrid(tgt_file):
    real_sampling_rate = 22050
    real_hop_length = 256
    tg = textgrid.TextGrid.fromFile(tgt_file)
    durations = []
    for phone in tg[1]:
        if phone.mark.strip() != "":
...
def duration(file):
    if "TextGrid" in str(file):
        durations = _duration_textgrid(file)
    elif "meta" in str(file):
        durations = _duration_meta(file)
    else:
        raise ValueError("Unknown file type")
    return np.mean(np.array(durations)*(256/22050)*1000)
